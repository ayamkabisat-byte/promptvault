...
export function FloatingSearch({ value, onChangeText, placeholder = 'Search…', accent = '#ff9138' }) {
  ...
  if (!open) {
    return (
      <Pressable style={styles.searchFab} onPress={() => setOpen(true)}>
  ...
  return (
    <View style={styles.searchPanel}>
      ...
      />
      <Pressable style={styles.searchClose} onPress={() => setOpen(false)}><Text style={styles.searchCloseText}>×</Text></Pressable>
    </View>
  );
}
...
const styles = StyleSheet.create({
  feed: { paddingHorizontal: 12, paddingTop: 8, paddingBottom: 112 },
  ...
  searchFab: {
    position: 'absolute', right: 14, top: 8, zIndex: 50,
    width: 46, height: 46, borderRadius: 23,
    backgroundColor: 'rgba(248,247,244,.96)',
  ...
  searchPanel: {
    position: 'absolute', left: 12, right: 12, top: 8, zIndex: 50,
