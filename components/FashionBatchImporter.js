...
import { ArrowLeft, CheckCircle2, ImageUp, LoaderCircle, Lock, Shirt, UploadCloud, XCircle } from "lucide-react";
...
export default function FashionBatchImporter() {
  ...
  const runImport = async () => {
    ...
    for (let index = 0; index < mapped.length; index += 1) {
      ...
      try {
        ...
        const path = `fashion/batch-1a/${styleId.toLowerCase()}-${row.slug}.webp`;
      ...
      <section className="fashion-import-panel">
        <div className="fashion-import-head">
          <div>
            ...
            <h1>Batch 1A Infographic Import</h1>
            <p>Extract ZIP terlebih dulu, lalu pilih seluruh 48 JPG sekaligus. Filename akan dipetakan otomatis ke F001–F048, dikonversi ke WebP, di-upload ke Supabase Storage, lalu URL-nya ditautkan ke Fashion Prompt.</p>
          </div>
        ...
        <label className="fashion-file-drop">
          <UploadCloud size={30} />
          <strong>{files.length ? `${files.length} file dipilih` : "Pilih semua gambar Batch 1A"}</strong>
          <span>JPEG, PNG, atau WebP · multi-select</span>
        ...
        {!!files.length && (
          <div className="fashion-map-preview">
            {mapped.map(({ file, styleId }) => (
              <div key={file.name} className={styleId ? "ok" : "bad"}><span>{styleId || "?"}</span><small>{file.name}</small></div>
            ))}
          </div>
        )}
        ...
        <div className="fashion-import-runbar">
          <button disabled={!files.length || running || !matchedCount} onClick={runImport}>
            {running ? <><LoaderCircle className="spin" size={16} /> Importing {progress}/{files.length}</> : <><UploadCloud size={16} /> Import matched images</>}
          </button>
          <button className="logout" onClick={() => supabase.auth.signOut()}>Sign out</button>
        </div>

        {running && <div className="fashion-progress"><i style={{ width: `${files.length ? (progress / files.length) * 100 : 0}%` }} /></div>}

        {!!results.length && (
          <section className="fashion-results">
            <header><strong>{successCount} uploaded</strong><span>{failCount} failed / skipped</span></header>
            {results.map((item, index) => <div key={`${item.file}-${index}`} className={item.ok ? "result-ok" : "result-bad"}>{item.ok ? <CheckCircle2 size={14} /> : <XCircle size={14} />}<span><b>{item.message}</b><small>{item.file}</small></span></div>)}
          </section>
        )}
      </section>
      <ImporterStyles />
