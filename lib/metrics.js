...
export async function trackSiteVisit(visitorId) {
  ...
  try {
    const { data, error } = await supabase.rpc("track_site_visit", { visitor_key: String(visitorId) });
...
export async function incrementPromptMetric(promptId, metric) {
  if (promptId === null || promptId === undefined || !["view", "copy", "favorite", "unfavorite"].includes(metric)) return null;
  try {
    const { data, error } = await supabase.rpc("increment_prompt_metric", {
      prompt_id_input: String(promptId),
      metric_input: metric,
    });
    if (error) return null;
    return data;
